...
static const CGFloat kAngleOffset = M_PI_2/1.1;
...
- (void)commonSetup
{
    ...
    for (int i = 0; i < self.count; i++) {
        UISnapBehavior *snap = [[UISnapBehavior alloc] initWithItem:self.items[i] snapToPoint:_startPoint];
...
{
    CGFloat firstAngle = M_PI + (M_PI_2 - self.angle) + index * self.angle*2;
    CGPoint startPoint = _startPoint;
...
- (void) showMenu:(CGPoint)fromPoint
{
...
{
    UISnapBehavior *snap = [[UISnapBehavior alloc] initWithItem:self.items[index] snapToPoint:_startPoint];
    snap.damping = self.sphereDamping;
