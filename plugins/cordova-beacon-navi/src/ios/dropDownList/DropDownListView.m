...
@implementation DropDownListView
{
    ...
    UIColor* _tableColor;
    UIColor* _tableBgColor;
}

- (void) setDropDownTextColor:(UIColor *)color withBackgroundColor:(UIColor *)bgColor
{
    _tableColor = color;
    _tableBgColor = bgColor;
}
...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate withTextColor:(UIColor *)textColor withBackgroundColor:(UIColor *)bgColor
{
    self = [super initWithFrame:frame];
    if (self) {
        _bgColor = bgColor;
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            [sectionBtn setTitleColor:_textColor forState:UIControlStateNormal];
...
{
    NSInteger numberOfRow = [self.dropDownDataSource numberOfRowsInSection:section];
    CGFloat height = 40*numberOfRow > kScreenHeight*2/3 - 150 ? kScreenHeight*2/3 -150 : 40*numberOfRow;
    if (!self.mTableView) {
        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(self.mSuperView.frame.origin.x, (self.frame.origin.y < 0? 0: self.frame.origin.y)+ self.frame.size.height , self.mSuperView.frame.size.width, self.mSuperView.frame.size.height - self.frame.origin.y - self.frame.size.height)];
        ...
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, height) style:UITableViewStylePlain];
        ...
        if(_tableBgColor)
            self.mTableView.backgroundColor = _tableBgColor;
        else
            self.mTableView.backgroundColor = _bgColor;
    ...
    CGRect rect = self.mTableView.frame;
    rect.origin.x = self.frame.origin.x + sectionWidth *section;
...
{
    UIButton *currentSectionBtn = (UIButton *)[self viewWithTag:SECTION_BTN_TAG_BEGIN + currentExtendSection];
    [currentSectionBtn setTitle:title forState:UIControlStateNormal];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(_tableColor)
        cell.textLabel.textColor = _tableColor;
    return cell;
}
